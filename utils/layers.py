...
# 	Author: Kumar Shridhar (kumar-shridhar)
# 	Date: 2019
# 	Availability: https://github.com/kumar-shridhar/PyTorch-BayesianCNN/tree/28d63e0d7dce28c674edbeaae24065f2599a3baf
...
from .distributions import Normal, Normalout, distribution_selector
...
class _ConvNd(nn.Module):
    """Bayesian convolutional layer with a distribution over each of the weights in the layer."""
    ...
    def params_count(self):
        # twice as many parameters (bias is not used)
        return 2 * (torch.tensor(self.qw_mean.shape).prod())# + torch.tensor(self.qb_mean.shape).prod())
...
class BayesianConv2d(_ConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(BayesianConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, _pair(0), groups)
    ...
    def forward(self, input):
        ...
        conv_qw_std = torch.sqrt(1e-8 + F.conv2d(input=input.pow(2), weight=torch.exp(self.log_alpha)*self.qw_mean.pow(2), stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups))

        # sample from output
        output = conv_qw_mean + conv_qw_std * (torch.randn(conv_qw_mean.size())).to(DEVICE)
...
class BayesianLinear(nn.Module):
    """Bayesian dense layer with a distribution over each of the weights in the layer."""
    ...
    def forward(self, input):
        ...
        # sample from output
        output = fc_qw_mean + fc_qw_si * (torch.randn(fc_qw_mean.size())).to(DEVICE)
    ...
    def params_count(self):
        # twice as many parameters (bias is not used)
        return 2 * (torch.tensor(self.qw_mean.shape).prod())# + torch.tensor(self.qw_mean.shape).prod())
